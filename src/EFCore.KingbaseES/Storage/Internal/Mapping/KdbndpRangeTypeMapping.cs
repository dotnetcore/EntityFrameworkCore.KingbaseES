...
public class KdbndpRangeTypeMapping : KdbndpTypeMapping
{
    ...
    /// </summary>
    public static KdbndpRangeTypeMapping Default { get; } = new();
    ...
    /// <summary>
    ///     For user-defined ranges, we have no <see cref="KdbndpDbType" /> and so the PG type name is set on
    ///     <see cref="KdbndpParameter.DataTypeName" /> instead.
    /// </summary>
    public virtual string? UnquotedStoreType { get; init; }

    /// <summary>
    ///     Constructs an instance of the <see cref="KdbndpRangeTypeMapping" /> class for a built-in range type which has a
    ///     <see cref="KdbndpDbType" /> defined.
    /// </summary>
    /// <param name="rangeStoreType">The database type to map</param>
    /// <param name="rangeClrType">The CLR type to map.</param>
    /// <param name="rangeKdbndpDbType">The <see cref="KdbndpDbType" /> of the built-in range.</param>
    /// <param name="subtypeMapping">The type mapping for the range subtype.</param>
    public static KdbndpRangeTypeMapping CreatBuiltInRangeMapping(
        string rangeStoreType,
        Type rangeClrType,
        KdbndpDbType rangeKdbndpDbType,
        RelationalTypeMapping subtypeMapping)
        => new(rangeStoreType, rangeClrType, rangeKdbndpDbType, subtypeMapping);

    /// <summary>
    ///     Constructs an instance of the <see cref="KdbndpRangeTypeMapping" /> class for a user-defined range type which doesn't have a
    ///     <see cref="KdbndpDbType" /> defined.
    /// </summary>
    ...
    /// <param name="rangeClrType">The CLR type to map.</param>
    /// <param name="subtypeMapping">The type mapping for the range subtype.</param>
    public static KdbndpRangeTypeMapping CreatUserDefinedRangeMapping(
        string quotedRangeStoreType,
        string unquotedRangeStoreType,
        Type rangeClrType,
        RelationalTypeMapping subtypeMapping)
        => new(quotedRangeStoreType, rangeClrType, rangeKdbndpDbType: KdbndpDbType.Unknown, subtypeMapping)
        {
            UnquotedStoreType = unquotedRangeStoreType
        };

    private KdbndpRangeTypeMapping(
        string rangeStoreType,
        Type rangeClrType,
        KdbndpDbType rangeKdbndpDbType,
        RelationalTypeMapping subtypeMapping)
        : base(rangeStoreType, rangeClrType, rangeKdbndpDbType)
    {
    ...
    private KdbndpRangeTypeMapping()
        : this("int4range", typeof(KdbndpRange<int>), KdbndpDbType.IntegerRange, subtypeMapping: null!)
    {
    }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </remarks>
    public virtual KdbndpRangeTypeMapping Clone(KdbndpDbType npgsqlDbType, RelationalTypeMapping subtypeTypeMapping)
    ...
    protected override RelationalTypeMapping Clone(RelationalTypeMappingParameters parameters)
        => new KdbndpRangeTypeMapping(parameters, KdbndpDbType, SubtypeMapping);
    ...
    {
        // Built-in range types have an KdbndpDbType, so we just do the normal thing.
        if (UnquotedStoreType is null)
        {
            Check.DebugAssert(KdbndpDbType is not KdbndpDbType.Unknown, "KdbndpDbType is Unknown but no PgDataTypeName is configured");
            base.ConfigureParameter(parameter);
            return;
        }

        Check.DebugAssert(KdbndpDbType is KdbndpDbType.Unknown, "PgDataTypeName is non-null, but KdbndpDbType is " + KdbndpDbType);
