...
using Kdbndp.EntityFrameworkCore.KingbaseES.Extensions.Internal;
...
public class KdbndpQueryableMethodTranslatingExpressionVisitor : RelationalQueryableMethodTranslatingExpressionVisitor
{
    ...
    private static readonly MethodInfo ILike2MethodInfo
        = typeof(KdbndpDbFunctionsExtensions).GetRuntimeMethod(
            nameof(KdbndpDbFunctionsExtensions.ILike), [typeof(DbFunctions), typeof(string), typeof(string)])!;
...
#pragma warning disable EF1001 // SelectExpression constructors are currently internal
        ...
        else
        {
            ...
            // Note also that with KingbaseES unnest, the output ordering is guaranteed to be the same as the input array. However, we still
                ...
                case PgBinaryExpression
                    {
                        ...
                        Right: { TypeMapping: KdbndpLTreeTypeMapping } ltree
                    }
