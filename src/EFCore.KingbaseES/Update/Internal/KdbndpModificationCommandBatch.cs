...
public class KdbndpModificationCommandBatch : ReaderModificationCommandBatch
{
    ...
    protected override void AddParameter(IColumnModification columnModification)
    {
        ...
        // sets. So we avoid adding KdbndpParameters for output parameters - we'll just retrieve and propagate the values below when
...
#pragma warning disable 618
        Debug.Assert(
            KdbndpReader.Statements.Count == ModificationCommands.Count,
            $"Reader has {KdbndpReader.Statements.Count} statements, expected {ModificationCommands.Count}");
...
#pragma warning disable 618
                    if (KdbndpReader.Statements[commandIndex].Rows != 1 && command.StoreStoredProcedure is null)
                    {
                        if (async)
                        {
                            await ThrowAggregateUpdateConcurrencyExceptionAsync(reader, commandIndex, 1, 0, cancellationToken)
