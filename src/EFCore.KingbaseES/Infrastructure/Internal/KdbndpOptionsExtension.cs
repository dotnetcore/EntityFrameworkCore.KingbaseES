...
public class KdbndpOptionsExtension : RelationalOptionsExtension
{
    ...
    /// <summary>
    ///     A lambda to configure Kdbndp options on <see cref="KdbndpDataSourceBuilder" />.
    /// </summary>
    public virtual Action<KdbndpDataSourceBuilder>? DataSourceBuilderAction { get; private set; }
...
#pragma warning restore CS0618
    ...
    public virtual KdbndpOptionsExtension WithDataSourceConfiguration(Action<KdbndpDataSourceBuilder> dataSourceBuilderAction)
    {
    ...
    ///     Returns a copy of the current instance configured with the specified range mapping.
    /// </summary>
    public virtual KdbndpOptionsExtension WithEnumMapping(
        Type clrType,
        string? enumName,
        string? schemaName,
...
#pragma warning restore CS0618
    ...
    public override void Validate(IDbContextOptions options)
    {
        ...
        var dataSource = DataSource
            ?? options.FindExtension<CoreOptionsExtension>()?.ApplicationServiceProvider?.GetService<KdbndpDataSource>();
        ...
        if (dataSource is not null)
        {
            if (ProvideClientCertificatesCallback is not null)
            {
                throw new InvalidOperationException(
                    "When passing an KdbndpDataSource to UseKdbndp(), call 'ProvideClientCertificatesCallback' on KdbndpDataSourceBuilder rather than in UseKdbndp().");
            }

            if (RemoteCertificateValidationCallback is not null)
            {
                throw new InvalidOperationException(
                    "When passing an KdbndpDataSource to UseKdbndp(), call 'RemoteCertificateValidationCallback' on KdbndpDataSourceBuilder rather than in UseKdbndp().");
            }

            if (ProvidePasswordCallback is not null)
            {
                throw new InvalidOperationException(
                    "When passing an KdbndpDataSource to UseKdbndp(), 'ProviderPasswordCallback' cannot be used in UseKdbndp(). See https://www.npgsql.org/doc/security.html for configuring passwords and token rotation on KdbndpDataSourceBuilder.");
            }
        }
        ...
        public override void PopulateDebugInfo(IDictionary<string, string> debugInfo)
        {
            ...
            foreach (var enumDefinition in Extension._enumDefinitions)
            {
                debugInfo[
                        ...
                        + nameof(KdbndpDbContextOptionsBuilder.MapEnum)
                        + ":"
