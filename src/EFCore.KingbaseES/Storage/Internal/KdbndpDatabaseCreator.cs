...
/// </summary>
public class KdbndpDatabaseCreator(
    ...
    : RelationalDatabaseCreator(dependencies)
{
    ...
    public override void Create()
    {
        using (var masterConnection = connection.CreateAdminConnection())
        {
            ...
            catch (KingbaseException e) when (e is { SqlState: "23505", ConstraintName: "sys_database_datname_index" })
            {
...
""");
    ...
    private async Task<bool> Exists(bool async, CancellationToken cancellationToken = default)
    {
        ...
        var unpooledCsb = new KdbndpConnectionStringBuilder(connection.ConnectionString) { Pooling = false, Multiplexing = false };
        ...
        var unpooledRelationalConnection =
             connection.CloneWith(unpooledCsb.ToString());
    ...
    private static bool IsDoesNotExist(KingbaseException exception)
    ...
    public override void CreateTables()
    {
        ...
        catch (KingbaseException e) when (e is { SqlState: "23505" })
        {
            // This occurs when two connections are trying to create the same database concurrently
            // (happens in the tests). Simply ignore the error.
        }

        if (reloadTypes && connection.DbConnection is KdbndpConnection npgsqlConnection)
        {
    ...
    private IReadOnlyList<MigrationCommand> CreateDropCommands()
    {
        var operations = new MigrationOperation[]
        {
            ...
            // Issue #775
            new KdbndpDropDatabaseOperation { Name = connection.DbConnection.Database }
        };
    ...
    private void ClearPool()
        => KdbndpConnection.ClearPool((KdbndpConnection)connection.DbConnection);
}
