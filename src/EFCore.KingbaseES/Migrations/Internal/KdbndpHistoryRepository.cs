...
/// </summary>
public class KdbndpHistoryRepository : HistoryRepository, IHistoryRepository
{
    ...
    public override IMigrationsDatabaseLock AcquireDatabaseLock()
    {
        ...
        return new KdbndpMigrationDatabaseLock(this);
    }
    ...
    private IModel EnsureModel()
    {
        if (_model == null)
        {
            ...
            conventionSet.Remove(typeof(KdbndpPostgresModelFinalizingConvention));
    ...
    bool IHistoryRepository.CreateIfNotExists()
    {
        ...
        // constraint violation (duplicate key value violates unique constraint "sys_type_typname_nsp_index").
        ...
        catch (KingbaseException e) when (e.SqlState is KingbaseErrorCodes.UniqueViolation
                                              or KingbaseErrorCodes.DuplicateTable
                                              or KingbaseErrorCodes.DuplicateObject)
        {
            return false;
        }
    }
...
""";
    ...
    public override IReadOnlyList<HistoryRow> GetAppliedMigrations()
    {
        ...
        catch (KingbaseException e) when (e.SqlState is KingbaseErrorCodes.InvalidCatalogName or KingbaseErrorCodes.UndefinedTable)
        {
            return [];
        }
    }
    ...
    /// <summary>
    ///     Always returns <see langword="true" /> for KingbaseES - it's difficult to reliably check if the migration history table
    ...
    private sealed class KdbndpMigrationDatabaseLock(IHistoryRepository historyRepository) : IMigrationsDatabaseLock
    {
        /// <summary>
        ///     This is an internal API that supports the Entity Framework Core infrastructure and not subject to
        ///     the same compatibility standards as public APIs. It may be changed or removed without notice in
