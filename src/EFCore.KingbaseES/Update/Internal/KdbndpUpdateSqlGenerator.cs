...
public class KdbndpUpdateSqlGenerator : UpdateSqlGenerator
{
    ...
    public override ResultSetMapping AppendUpdateOperation(
        ...
        out bool requiresTransaction)
    {
        // The default implementation adds RETURNING 1 to do concurrency check (was the row actually updated), but in KingbaseES we check
        // the per-statement row-affected value exposed by Kdbndp in the batch; so no need for RETURNING 1.
        var name = command.TableName;
        var schema = command.Schema;
        var operations = command.ColumnModifications;
        ...
        string? schema)
    {
        ...
        {
            Check.DebugAssert(
                columnModification.TypeMapping is KdbndpOwnedJsonTypeMapping,
                "ColumnModification with JsonPath but non-KdbndpOwnedJsonTypeMapping");

            if (columnModification.TypeMapping.StoreType is "json")
            {
                throw new NotSupportedException(
                    "Cannot perform partial update because the KingbaseES 'json' type has no json_set method. Use 'jsonb' instead.");
            }
    ...
    public override ResultSetMapping AppendDeleteOperation(
        ...
        out bool requiresTransaction)
    {
        // The default implementation adds RETURNING 1 to do concurrency check (was the row actually deleted), but in KingbaseES we check
        // the per-statement row-affected value exposed by Kdbndp in the batch; so no need for RETURNING 1.
        var name = command.TableName;
        var schema = command.Schema;
        ...
        out bool requiresTransaction)
    {
        ...
        // KingbaseES supports neither a return value nor a result set with stored procedures, only output parameters.
