...
public class KdbndpSqlTranslatingExpressionVisitor : RelationalSqlTranslatingExpressionVisitor
{
    ...
    private static readonly MethodInfo EscapeLikePatternParameterMethod =
        typeof(KdbndpSqlTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ConstructLikePatternParameter))!;
    ...
    protected override Expression VisitUnary(UnaryExpression unaryExpression)
    {
        switch (unaryExpression.NodeType)
        {
            ...
            // We map both IPAddress and KdbndpInet to PG inet, and translate many methods accepting KdbndpInet, so ignore casts from
            // IPAddress to KdbndpInet.
            // On the KingbaseES side, cidr is also implicitly convertible to inet, and at the ADO.NET level KdbndpCidr has a similar
            // implicit conversion operator to KdbndpInet. So remove that cast as well.
            case ExpressionType.Convert
                when unaryExpression.Type == typeof(KdbndpInet)
                && (unaryExpression.Operand.Type == typeof(IPAddress) || unaryExpression.Operand.Type == typeof(KdbndpCidr)):
                return Visit(unaryExpression.Operand);
        }
    ...
    protected override Expression VisitNew(NewExpression newExpression)
    {
        ...
        // This is notably done to support row value comparisons: WHERE (x, y) > (3, 4) (see e.g. KdbndpDbFunctionsExtensions.GreaterThan)
    ...
    {
        // Docs: https://www.KingbaseES.org/docs/current/functions-conditional.html#FUNCTIONS-GREATEST-LEAST
