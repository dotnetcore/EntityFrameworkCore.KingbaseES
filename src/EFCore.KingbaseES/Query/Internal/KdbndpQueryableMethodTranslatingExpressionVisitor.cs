...
/// </summary>
public class KdbndpQueryableMethodTranslatingExpressionVisitor : RelationalQueryableMethodTranslatingExpressionVisitor
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public KdbndpQueryableMethodTranslatingExpressionVisitor(
        QueryableMethodTranslatingExpressionVisitorDependencies dependencies,
        RelationalQueryableMethodTranslatingExpressionVisitorDependencies relationalDependencies,
        QueryCompilationContext queryCompilationContext)
        : base(dependencies, relationalDependencies, queryCompilationContext)
    {
        _typeMappingSource = (KdbndpTypeMappingSource)relationalDependencies.TypeMappingSource;
        _sqlExpressionFactory = (KdbndpSqlExpressionFactory)relationalDependencies.SqlExpressionFactory;
    }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected KdbndpQueryableMethodTranslatingExpressionVisitor(KdbndpQueryableMethodTranslatingExpressionVisitor parentVisitor)
        : base(parentVisitor)
    {
...
#pragma warning disable EF1001
        ...
        else
        {
            ...
            // Note also that with KingbaseES unnest, the output ordering is guaranteed to be the same as the input array, so we don't need
            // to add ordering like in most other providers (https://www.KingbaseES.org/docs/current/functions-array.html)
...
#pragma warning restore EF1001
    ...
    protected override ShapedQueryExpression TransformJsonQueryToTable(JsonQueryExpression jsonQueryExpression)
    {
        ...
        Check.DebugAssert(jsonTypeMapping is KdbndpOwnedJsonTypeMapping, "JSON column has a non-JSON mapping");
...
#pragma warning restore EF1001 // Internal EF Core API usage.
        ...
        [NotNullWhen(true)] out TableExpression? tableExpression)
    {
        ...
        // main table is allowed) - see KdbndpQuerySqlGenerator.VisitUpdate.
        ...
        for (var i = 0; i < selectExpression.Tables.Count; i++)
        {
            ...
            // For inner joins, if the predicate contains a reference to the main table, KdbndpQuerySqlGenerator will lift the predicate
    ...
    /// <summary>
    ///     KingbaseES array indexing is 1-based. If the index happens to be a constant, just increment it. Otherwise, append a +1 in the
    ///     SQL.
    /// </summary>
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected class KdbndpInferredTypeMappingApplier : RelationalInferredTypeMappingApplier
    {
        private readonly KdbndpTypeMappingSource _typeMappingSource;
        private readonly KdbndpSqlExpressionFactory _sqlExpressionFactory;
        ...
        ///     doing so can result in application failures when updating to a new Entity Framework Core release.
        /// </summary>
        public KdbndpInferredTypeMappingApplier(
            IModel model,
